...
class UserSignUpForm(rio.Component):
    ...
    async def on_sign_up(self) -> None:
        ...
        if (
            ...
            or not self.name_sign_up
            or not self.email_sign_up
        ...
        atsign=self.email_sign_up.find('@')
        if atsign > 0 and \
             self.email_sign_up[atsign:].find('.') > 0:
           # Good email
           pass
        else:
           self.email_valid = False
           self.error_message = "You need a valid email-address"
           return
        ...
        #)
        user_info = {"username":self.username_sign_up,
                     "name":self.name_sign_up,
                     "email":self.email_sign_up,
                     "raw_password":self.password_sign_up}
        ...
        # Now redirect to the pending approval page
    ...
    def build(self) -> rio.Component:
        ...
        return rio.Card(
            rio.Column(
                ...
                rio.TextInput(
                    text=self.bind().email_sign_up,
                    label="Email*",
                    is_valid=self.email_valid,
                ),
                rio.TextInput(
                    text=self.bind().name_sign_up,
                    label="Real Name*",
                    is_valid=self.email_valid,
                ),
                rio.TextInput(
